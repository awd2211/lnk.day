...
// Mock data
const mockSlackInstallation: SlackInstallation = {
  installed: true,
...
const mockSlackChannels: SlackChannel[] = [
  { id: 'C1234567890', name: 'marketing', isPrivate: false },
  { id: 'C0987654321', name: 'engineering', isPrivate: false },
  { id: 'C1111111111', name: 'notifications', isPrivate: false },
  { id: 'C2222222222', name: 'private-team', isPrivate: true },
];

const mockTeamsInstallations: TeamsInstallation[] = [
  {
    id: '1',
    name: '市场团队通知',
    webhookUrl: 'https://acme.webhook.office.com/webhookb2/xxx...',
    isActive: true,
    settings: {
      notifyOnLinkCreate: true,
      notifyOnMilestone: true,
      notifyOnAlert: false,
      notifyOnWeeklyReport: true,
      milestoneThresholds: [1000, 10000],
    },
    lastTestedAt: '2024-01-20T14:30:00Z',
...
export default function NotificationIntegrationsPage() {
  ...
  const { data: slackInstallation } = useQuery({
    ...
    queryFn: async () => mockSlackInstallation,
  });

  const { data: slackChannels } = useQuery({
    queryKey: ['slack-channels'],
    queryFn: async () => mockSlackChannels,
    enabled: slackInstallation?.installed,
  });

  const { data: teamsInstallations } = useQuery({
    queryKey: ['teams-installations'],
    queryFn: async () => mockTeamsInstallations,
  });

  // State for forms
  const [slackSettings, setSlackSettings] = useState(slackInstallation?.settings || {
    notifyOnLinkCreate: true,
    notifyOnMilestone: true,
    notifyOnAlert: true,
    notifyOnWeeklyReport: true,
  ...
  const [selectedChannel, setSelectedChannel] = useState(slackInstallation?.defaultChannel || '');
  ...
  const handleConnectSlack = () => {
    // In real app, redirect to Slack OAuth URL
    window.location.href = '/api/slack/oauth/install?redirectUrl=' + encodeURIComponent(window.location.href);
  };
  ...
  const handleSaveSlackSettings = async () => {
    // In real app, call API
    console.log('Saving Slack settings:', { channel: selectedChannel, settings: slackSettings });
  };

  const handleTestSlackConnection = async () => {
    setTestingConnection('slack');
    // In real app, call API to send test message
    await new Promise(resolve => setTimeout(resolve, 2000));
    setTestingConnection(null);
  };

  const handleAddTeamsInstallation = async () => {
    // In real app, call API
    console.log('Adding Teams installation:', newTeamsForm);
  ...
  const handleTestTeamsConnection = async (installationId: string) => {
  ...
  const handleDeleteTeamsInstallation = async (installationId: string) => {
    // In real app, call API
    console.log('Deleting Teams installation:', installationId);
  };
  ...
  const toggleMilestone = (threshold: number, settings: any, setSettings: (s: any) => void) => {
    ...
    if (current.includes(threshold)) {
      ...
      setSettings({
        ...settings,
        milestoneThresholds: [...current, threshold].sort((a, b) => a - b),
      });
    }
  };
  ...
  return (
    <Layout
      title="通知集成"
      description="配置 Slack、Microsoft Teams 等第三方通知渠道"
    >
      <Tabs defaultValue="slack" className="space-y-6">
        <TabsList>
          ...
          <TabsTrigger value="teams" className="flex items-center gap-2">
            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20.625 8.073c.574 0 1.125.228 1.531.634.406.406.634.957.634 1.531v4.524a2.165 2.165 0 0 1-2.165 2.165h-3.18v3.18a2.165 2.165 0 0 1-2.164 2.164H9.14a2.165 2.165 0 0 1-2.165-2.165v-3.179H3.796a2.165 2.165 0 0 1-2.165-2.165v-4.524c0-.574.228-1.125.634-1.531a2.165 2.165 0 0 1 1.531-.634h3.18V4.893a2.165 2.165 0 0 1 2.164-2.165h6.141a2.165 2.165 0 0 1 2.165 2.165v3.18h3.179zm-3.179 0V4.893a.722.722 0 0 0-.722-.722H9.14a.722.722 0 0 0-.722.722v3.18h3.179a2.165 2.165 0 0 1 2.165 2.165v3.179h3.684z"/>
            </svg>
            Microsoft Teams
          </TabsTrigger>
        </TabsList>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between rounded-lg border bg-muted/50 p-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={handleDisconnectSlack}>
                        断开连接
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end">
                    <Button onClick={handleSaveSlackSettings}>保存设置</Button>
                  </div>
                </div>
              ) : (
        ...
        <TabsContent value="teams" className="space-y-6">
          <Card>
            ...
            <CardContent>
              {teamsInstallations && teamsInstallations.length > 0 ? (
                <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleDeleteTeamsInstallation(installation.id)}
                          >
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newTeamsForm.name || !newTeamsForm.webhookUrl}
            >
              添加
            </Button>
          </DialogFooter>
      ...
      >
        <DialogContent className="max-w-lg">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {milestoneOptions.map((threshold) => (
                      <Button
                        key={threshold}
                        variant={
                          editingTeamsInstallation.settings.milestoneThresholds.includes(
                            threshold
                          )
                            ? 'default'
                            : 'outline'
          ...
          <DialogFooter>
            ...
            <Button onClick={() => setEditingTeamsInstallation(null)}>保存</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Layout>
