...
export function useAutomationWebhooks(platform?: AutomationPlatform) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get('/automation/webhooks', { params });
...
export function useAutomationWebhook(id: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      const { data } = await api.get(`/automation/webhooks/${id}`);
...
export function useAutomationPlatforms() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/automation/platforms');
...
export function useAutomationEvents() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/automation/events');
...
export function useCreateWebhook() {
  ...
  return useMutation({
    mutationFn: async (dto: CreateWebhookDto) => {
      const { data } = await api.post('/automation/webhooks', dto);
      return data.webhook as AutomationWebhook;
    },
    onSuccess: () => {
...
export function useUpdateWebhook() {
  ...
  return useMutation({
    mutationFn: async ({ id, dto }: { id: string; dto: UpdateWebhookDto }) => {
      const { data } = await api.put(`/automation/webhooks/${id}`, dto);
      return data.webhook as AutomationWebhook;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['automation'] });
...
export function useToggleWebhook() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { data } = await api.post(`/automation/webhooks/${id}/toggle`);
      return data.webhook as AutomationWebhook;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['automation'] });
...
export function useTestWebhook() {
  return useMutation({
    mutationFn: async (id: string) => {
      const { data } = await api.post(`/automation/webhooks/${id}/test`);
