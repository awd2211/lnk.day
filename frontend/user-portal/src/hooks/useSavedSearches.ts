...
export function useUpdateSavedSearch() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<CreateSavedSearchDto> }) => {
      const response = await api.patch<SavedSearch>(`/search/saved/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['saved-searches'] });
...
export function useApplySavedSearch() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await api.post<{ results: any[] }>(`/search/saved/${id}/apply`);
      return response.data;
    },
    onSuccess: (_, id) => {
...
export function useSetDefaultSearch() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await api.post<SavedSearch>(`/search/saved/${id}/set-default`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['saved-searches'] });
...
export function useDuplicateSavedSearch() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await api.post<SavedSearch>(`/search/saved/${id}/duplicate`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['saved-searches'] });
